...
<p align="center">
  ...
  单二进制即刻启动，除模型与自选 MCP 外零网络暗流。透明结对拒绝黑盒盲等，持久记忆终结失忆轮回，更可自主沉淀模板与子角色——让终端智能真正随你一同进化。
</p>
...
- **能力闭环，自主进化**：它不仅被动听令，更能识别高频重复工作，主动为你沉淀专用的 `:prompt` 快捷模板，甚至能为复杂的特定场景自主编写专职的 **Subagent 角色卡**——随着日常使用，整套系统都在为你量身自我塑形。

**纯粹、克制、恪守边界。** 你的配置、会话、记忆、日志与产物完全封存于本地。除你指定的大模型 API 与自选 MCP 服务外，Kana 绝不向外发起任何网络暗流，更无任何埋点与遥测——你的终端，由你绝对掌控。
...
| 🧬 | **能力闭环，自主进化** | **越用越契合你的专属系统**。识别高频工作流主动为你沉淀 `:prompt` 快捷模板，甚至能为特定复杂场景自主编写专职的 Markdown Subagent 角色卡，实现能力自我塑形。 |
| 🛡️ | **零遥测暗流，本地绝对主权** | **纯净单二进制，毫秒冷启**。除你指定的模型 API 与自选 MCP 外无任何网络暗流与埋点遥测；配置、会话与产物严格以 `0600`/`0700` 权限封存于本地。 |
