...
提交：`refactor(tests): finish test suite cleanup`

- 测试文件：131 -> 162；聚合文件拆成模块或行为所有者后文件数增加，但可从文件名直接定位失败职责。
- 测试行数：38,253 -> 36,588，减少 1,665 行（4.4%）；包含领域 fixture 在内的全部 `tests/**/*.ts` 为 38,802 -> 37,567，减少 1,235 行。
- 测试用例：1,117 -> 1,075，净移除 42 个跨层重复、实现细节重复或错误归属用例；各任务均记录了具体删除依据。
- 大文件分布：超过 500 行的测试文件由 22 个降至 14 个；最大 20 个文件占比由 49.1% 降至 40.4%。保留的大文件均已记录单一状态机或组件所有权。
- 名称审查：只剩 2 组重复 leaf title，分别属于 MarkdownBlock/TextBlock cache 和 MCP/Skill manager activation；完整 suite 名不同，且验证不同组件契约。
- Fixture 审查：最终将 3 份相同 TUI Agent stub 并入 `app-fixture.ts`；剩余 `createTempEnv`、`waitFor`、controlled model/stream 均有明确路径、调度或协议差异。
- 长期约束：更新 `AGENTS.md`，明确测试要求不是 append 要求；新增前先找最低层所有者和现有覆盖，完整矩阵只放在最低层，并对超过 600 行或 20 个用例的文件触发归属审查。
- 计划复核：14 个任务全部完成，没有未解释的待办。
- 验证：最终 fixture 目标测试 15 个通过；`bun run check` 通过，完整套件 1,075 个测试通过。
