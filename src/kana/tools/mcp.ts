...
export function createMcpTools(manager: McpToolRegistry): Tool[] {
  ...
  return [
    {
      name: "mcp_activate",
      description: `Load the tool catalog and full input schemas for an enabled MCP server. This only reads the catalog; it does not enable servers or grant permissions. Load again whenever schemas are needed, including after context compaction. Follow nextOffset to read more tools; read any saved artifact to obtain complete schemas.\n\nAvailable MCP servers:\n${catalog}`,
      ...
      execute({ name, offset = 0, limit = 20 }) {
        if (!manager.catalog.some((server) => server.name === name)) {
          throw new Error(`MCP server "${name}" is not available.`);
        }
        const tools = manager.tools.filter((tool) => tool.source.serverId === name);
    ...
    },
    {
      name: "mcp_call",
      description:
        "Call a tool on an enabled MCP server. Use mcp_activate to read the tool's input schema before calling it. The arguments object must satisfy that schema. Loading a catalog does not change the available servers or tools.",
      parameters: strictObject({
        server: Type.String(),
        tool: Type.String(),
      ...
      async execute({ server, tool: name, arguments: args }, context) {
        const tool = manager.getTool(server, name);
        if (!tool) {
