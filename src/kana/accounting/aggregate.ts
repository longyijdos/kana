...
function summarize(
  ...
  const models = new Map<
    string,
    { provider: string; model: string; runCount: number; usage?: ModelUsage }
  >();
  for (const record of records) {
    ...
    const model = models.get(key) ?? { ...record.model, runCount: 0 };
    model.runCount += 1;
  ...
  return {
    ...
    models: [...models.values()].sort(
      (left, right) => (right.usage?.totalTokens ?? 0) - (left.usage?.totalTokens ?? 0),
    ),
  };
}
