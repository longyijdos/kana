...
- User image attachments become classic Responses `input_image` items with self-contained data URLs. When `image_input = false`, Kana sends no image bytes and appends an explicit text omission marker instead.
