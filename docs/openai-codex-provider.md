...
The first HTTP `401` triggers one credential refresh and retries with the new token. HTTP 408, 429, 5xx, and network failures use bounded exponential-backoff retries; Agent cancellation and inactivity timeout remain distinct and stop immediately. Retained HTTP error bodies are bounded to 16 KiB. Responses `error` and `response.failed` events with recognized transient overload, server, internal, temporary-unavailability, or rate-limit identities are retried with the same `max_retries` budget, including the exact provider backoff hint when present. A stream retry is allowed only before any assistant content or provider-hosted search activity has been emitted; once output starts, or when the failure is validation/protocol-related, Kana fails without replaying the request. A recognized context-window rejection maps to `ContextWindowExceededError`, allowing one safe Agent compaction recovery when no output has started.

Diagnostics use stable request start/completion/failure, HTTP retry, authentication-refresh start/end, and stream-recovery start/end events. Every record includes provider, model, protocol, phase, and outcome; retries and failures add a fixed Kana `errorCode`, safe `errorType`, attempt, stream event type, safe upstream `providerCode`, or HTTP status where relevant. Authentication refresh, ordinary HTTP retry, and Responses stream recovery remain separate lifecycle boundaries even though HTTP and stream retries share one budget. Logs never contain error messages, streamed content, tokens, account IDs, headers, prompts, complete tool arguments, or response bodies.
