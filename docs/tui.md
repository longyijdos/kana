...
OpenAI Codex `web_search_call` items are provider-hosted activity, so they create neither local approvals nor ToolRuntime execution. The TUI renders each call separately as `Searching the web`, `Searched the web`, `Opened a web page`, or `Searched within a web page`; calls are not aggregated today. The status phase is `searching` while a call is active. Provider Markdown links in the final answer render unchanged, without back-inserting citation numbers or appending a `Sources` block.
