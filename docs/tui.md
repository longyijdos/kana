...
助手消息和内存查看器使用轻量 Markdown 渲染：标题、列表、引用、代码围栏、部分 inline 样式、表格行、链接/图片文本和有限 HTML 规范化。Shiki 语法高亮在后台预加载；未加载时代码以普通文本显示。工具块对 list/glob/grep/read 显示摘要，对 write/edit 显示高亮 diff，对 bash 显示尾部输出；write 审批和工具块会区分新建与覆盖；长输出可在查看器中滚动。
