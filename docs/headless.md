...
New and resumed executions use the same `KanaConversationHost` and `ConversationRuntime` as the TUI, so model, prompt, tools, session, and product policy remain shared. Their ownership and cleanup contract belongs to [Conversation runtime](conversation-runtime.md); persisted state belongs to [Sessions and memory](sessions-and-memory.md).

The headless frontend keeps startup, execution, projection, and public protocol concerns separate. `startHeadless` resolves input, starts product resources, and closes them in order; the runner submits one turn or bounded Goal with fail-closed approval and owns the external abort listener and soft deadline. A stateful output projector alone holds terminal presentation state and routes JSONL or human output, while `protocol.ts` owns public event types, schema versioning, and wire conversion. Output-write and cleanup failures remain observable.

After the conversation runtime closes, shared host cleanup settles automatic memory work before closing MCP; the exact resource order is documented in [Conversation runtime](conversation-runtime.md). A `remember` entry has already reached daily staging at that point, so cancellation leaves it intact without committing a partial durable-memory transaction.
