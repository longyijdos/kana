...
The headless frontend keeps startup, execution, lifecycle, projection, and protocol concerns separate. `startHeadless` resolves input, starts the host and MCP, and closes runtime and host resources in order. The runner constructs `ConversationRuntime`, installs fail-closed approval, and sequences one turn or Goal. Its lifecycle owns the external abort listener and complete-run soft deadline, removing both at one cleanup boundary. The run-output projector alone holds terminal state and routes JSONL or human output, while `protocol.ts` owns public event types, schema versioning, and wire conversions. JSONL and human modes therefore consume the same run outcome without duplicating execution, and output-write or cleanup failures remain observable.
