...
At TUI startup, Kana reads `mcpServers` from the separate `mcp.json`, starts the stdio manager, and injects discovered remote tools into the main Agent as `additionalTools`. Agent recreation for `/new`, `/fork`, `/resume`, and Skill refresh reuses the same startup tool set. Memory-consolidation Agents bypass this factory and therefore never receive MCP tools. `McpManager` still requires only `connect/listTools/callTool/close`, while the adapter requires only an `McpToolCaller`, so the stable stdio client and future stateless, Streamable HTTP, or SSE clients continue to share the management and tool boundaries.
