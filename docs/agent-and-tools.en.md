...
Kana can now parse `mcpServers` from a separate `mcp.json` and create a stdio manager, but the TUI does not yet start it or inject remote tools into an Agent. `src/mcp` can already manage multiple servers and convert discovered remote tools into the generic `Tool` above. `McpManager` only requires clients to implement `connect/listTools/callTool/close`, while the adapter only requires an `McpToolCaller`. The stable stdio client and future stateless, Streamable HTTP, or SSE clients can therefore share the management and tool boundaries.
