...
An active goal uses its own `goal` runtime-context source. That snapshot contains the objective and terminal-update guidance, but omits the controller ID, admitted-run count, and configured limit so runtime scheduling does not become task semantics. Automatic compaction reprojects this objective after the summary. The snapshot may remain in append-only session history, but the authorization and controller are process-local; resuming the session without an active controller projects the source inactive and does not recreate the goal.
