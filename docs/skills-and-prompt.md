...
Before every model step, the Agent resolves Kana's dynamic environment, Job, todo, and Goal context together with the currently available capability sections. The stable system prefix remains unchanged so provider prompt caches can reuse it. Dynamic-state transitions belong to [Agent runtime](agent-runtime.md), runtime Goal ownership to [Conversation runtime](conversation-runtime.md), and executable capability rules to [Tools and execution](tools.md).

`--clean` bypasses global and project Skill discovery, `skills.toml` activation reads, both memory scopes, and both `AGENTS.md` scopes. The stable system prompt then contains the built-in assistant instructions and runtime-context protocol; dynamic environment context remains available. The Agent registers neither `remember` nor MCP gateways. `/skills` and `/memory` report that they are unavailable in clean mode. `.env`, provider/model selection, and other runtime configuration still follow the normal startup path, but a `/model` selection remains local to the temporary process.
...
The environment block is one identified runtime-context source. Change detection, active/inactive transitions, journal timing, and hidden transcript behavior belong to [Agent runtime](agent-runtime.md).

Compaction projection for identified runtime context belongs to [Agent runtime](agent-runtime.md); its persisted checkpoint representation belongs to [Sessions and memory](sessions-and-memory.md).

Kana supplies active Goal state through a separate `goal` source and advertises `update_goal` only while that capability is authorized. Goal admission and process-local control state belong to [Conversation runtime](conversation-runtime.md).
