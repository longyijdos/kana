...
The Agent compares each dynamic section with the latest history snapshot having the same `source`. It appends and journals a new message only when the content changes; unchanged values cost no extra history tokens. These internal messages are persisted for resume and hidden from the transcript. Compaction does not summarize them; when a checkpoint covers one, the latest covered snapshot for each source is projected immediately after the summary.
