...
| `tui.group_tool_calls` | Boolean | `true` | Group list/glob/grep/read calls across consecutive tool-only model turns into one exploration activity and adjacent provider-hosted web actions into one search activity; when disabled, render every call separately. |
...
`hyperlinks` is permission rather than a force switch: even when it is `true`, Kana emits OSC 8 only for terminals with confirmed support and keeps the URL visible when capability is unknown; `false` always uses the text fallback. By default, `smooth_text_streaming` changes only visible-text pacing and never backpressures the provider or Agent. When disabled, the TUI still coalesces terminal repaints but no longer subdivides provider text snapshots. `collapse_long_pastes` affects only editor presentation and editing: the complete pasted text is still submitted, queued, and restored from input history. `group_tool_calls` affects grouping only. Regardless of this setting, provider reasoning and local-tool argument streaming share one `thinking` activity, while local tool blocks appear only when execution actually starts. When grouping is enabled, consecutive tool-only model turns append executing list/glob/grep/read calls to one exploration phase, and completed calls do not finalize it while the Agent may request more exploration work. Its timer excludes argument parsing, queue, and approval time; when a queued non-exploration call follows, the phase freezes after the last exploration call and before approval. Visible assistant text, a non-exploration tool, a failed exploration call, context compaction, or the end of the Agent run also finalizes the phase; failures retain standalone error output. Grouped reads use final filenames unless collisions require shortest unique suffixes. A hosted web group starts with an empty active header and appends only completed provider actions, avoiding a provisional generic `Search`; reasoning and later hosted actions in the same response keep it active until visible assistant content, a local tool, or response completion establishes a terminal boundary. An unresolved action becomes `Web search stopped` on user abort and `Web search failed` on provider failure, including when restored from history. When `daily_retention_days` is commented out or omitted, daily memory is not pruned. Logs always write under `<KANA_HOME>/logs`; the directory is not configurable and log output never goes through the terminal, so it cannot disrupt TUI repainting. `max_turns` accepts only `-1` or a positive integer; `parallel_tool_calls`, provider `web_search`/`image_input`, `hyperlinks`, `smooth_text_streaming`, `collapse_long_pastes`, and `group_tool_calls` must be Boolean; `tool_deadline_ms`, `max_tokens`, and optional `context_limit` require positive integers, `timeout_ms` and `max_retries` are validated as finite numbers, and the two `memory` quantity fields require positive integers.
