...
`hyperlinks` is permission rather than a force switch: even when it is `true`, Kana emits OSC 8 only for terminals with confirmed support and keeps the URL visible when capability is unknown; `false` always uses the text fallback. By default, `smooth_text_streaming` changes only visible-text pacing and never backpressures the provider or Agent. When disabled, the TUI still coalesces terminal repaints but no longer subdivides provider text snapshots. `collapse_long_pastes` affects only editor presentation and editing: the complete pasted text is still submitted, queued, and restored from input history. `group_tool_calls` affects presentation only: consecutive tool-only model turns append list/glob/grep/read calls to one local exploration phase, and completed calls do not finalize it while the Agent may request more. The phase header starts with the first streamed call, while concrete entries wait for complete arguments; canceling provisional arguments retains an empty stopped phase instead of showing a placeholder entry. Visible assistant text, a non-exploration tool, a failed exploration call, context compaction, or the end of the Agent run finalizes the phase; failures retain standalone error output. Grouped reads use final filenames unless collisions require shortest unique suffixes. A hosted web group likewise starts with an empty active header and appends only completed provider actions, avoiding a provisional generic `Search`; it remains active while the response can append actions, then becomes terminal when visible assistant content starts or the response settles. An unresolved action becomes `Web search stopped` on user abort and `Web search failed` on provider failure, including when restored from history. When `daily_retention_days` is commented out or omitted, daily memory is not pruned. Logs always write under `<KANA_HOME>/logs`; the directory is not configurable and log output never goes through the terminal, so it cannot disrupt TUI repainting. `max_turns` accepts only `-1` or a positive integer; `parallel_tool_calls`, provider `web_search`/`image_input`, `hyperlinks`, `smooth_text_streaming`, `collapse_long_pastes`, and `group_tool_calls` must be Boolean; `tool_deadline_ms`, `max_tokens`, and optional `context_limit` require positive integers, `timeout_ms` and `max_retries` are validated as finite numbers, and the two `memory` quantity fields require positive integers.
