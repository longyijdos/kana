...
| `tui.group_tool_calls` | Boolean | `true` | Group list/glob/grep/read calls from each model tool-call batch into one `Explored` activity and adjacent provider-hosted web actions into one search activity; when disabled, render every call separately. |
