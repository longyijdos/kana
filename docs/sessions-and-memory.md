...
Dynamic prompt state uses an internal user-role message with `provenance.kind: "runtime_context"` and a non-empty `source`. The Agent journals a new snapshot only when that source's content changes. These messages remain in JSONL so resume reconstructs the same model context, but restored TUI history hides them because they are not human input.
