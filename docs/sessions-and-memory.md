...
This document owns persisted session and memory formats, recovery, artifacts, and retention. For live session-resource ownership and session transitions, see [Conversation runtime](conversation-runtime.md). For the Agent's model-context projection and compaction loop, see [Agent runtime](agent-runtime.md).
...
Dynamic prompt state is persisted as internal user-role messages with `provenance.kind: "runtime_context"` and a non-empty `source`. Each changed identified state is append-only in JSONL, while restored TUI history hides these internal messages. Active/inactive transitions and model-context projection belong to [Agent runtime](agent-runtime.md).
...
Compaction never removes original images from session JSONL. Whether summary generation receives image bytes or only metadata follows the active model's image capability and is documented in [Agent runtime](agent-runtime.md).
...
- Live resource disposal and host shutdown order belong to [Conversation runtime](conversation-runtime.md); this section covers the journal and artifact effects that remain durable.
...
- Resume reconstructs committed journal messages, the latest context checkpoint, and the latest todo state. Process-local inbox, wake, Goal, and user-task state is intentionally outside the session format; see [Conversation runtime](conversation-runtime.md).
...
The consolidation Agent uses its own `[memory.agent]` policy and no bash, file, or `remember` tools. It inherits `[agent.model]` unless `[memory.agent.model]` overrides individual fields, and web search and image input stay disabled. Incremental mode exposes only `read_memory`, `edit_memory`, and `replace_memory`, and its input contains only current durable memory and the new entries from this batch. It does not scan historical daily files, preventing inference from unprovided history.
