...
On first write, an explicit title wins. Otherwise Kana uses the first user-role message that is neither recovery nor runtime context, then collapses whitespace and truncates it to at most 80 JavaScript characters. With no usable text, the title is `Untitled session`.
