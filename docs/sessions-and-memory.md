...
The consolidation Agent uses its independent `[memory.agent]` policy and `[memory.agent.model]` selection, and has no bash, file, or `remember` tools. The default is the cheaper `deepseek-flash` with web search and image input disabled. Incremental mode exposes only `read_memory`, `edit_memory`, and `replace_memory`, and its input contains only current durable memory and the new entries from this batch. It does not scan historical daily files, preventing inference from unprovided history.
