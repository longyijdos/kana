...
- `/model` 只在空闲时完成切换。Kana 保留当前消息和 context checkpoint，并根据解析后的 `[agent]` 选择构造候选主 Agent。普通模式只原子保存 `agent.provider`、`agent.model` 与可选推理强度；Clean 模式把已校验文档保留在进程内。全部成功后才替换当前 Agent 和状态栏身份；构造或持久化失败会保留旧 Agent 与旧配置并在 transcript 显示错误。`[memory.agent]` 和现有记忆合并 scheduler 不会改变。该选择作用于后续主 Agent 及其上下文压缩；Clean 模式只在当前进程内生效。
