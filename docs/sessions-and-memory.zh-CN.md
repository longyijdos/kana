...
合并 Agent 使用自己的 `[memory.agent]` 策略，并且没有 bash、文件工具或 `remember`。除 `[memory.agent.model]` 覆盖的字段外，它继承 `[agent.model]`，网页搜索和图片输入保持关闭。增量模式仅提供 `read_memory`、`edit_memory`、`replace_memory`，且输入只包含当前长期记忆和本批新条目。它不扫描历史 daily 文件，避免把未提供的上下文推断进记忆。
