...
Clean mode retains only items 2 and 5. It does not scan Skill paths, read memory, or construct the automatic memory-consolidation scheduler. The host still supplies current runtime configuration to the Agent, so provider/model, context limit, output ceiling, and tool deadline remain identical to normal mode; `schedule_wake` also remains available when supported by the frontend.
