...
When the TUI starts, `startTui` loads runtime configuration, the separate MCP configuration, and the approval allowlist; creates the current session; connects MCP servers and discovers their initial tools; then constructs `KanaTuiApp`. It injects session I/O, Skill activation, memory compaction, and an Agent factory carrying MCP tools as callbacks. The app therefore coordinates user flows without knowing JSONL, TOML, MCP transports, or other storage and protocol details.
...
The manager currently freezes the tool list discovered at startup and does not process `notifications/tools/list_changed`. The `kana` layer parses `mcpServers` from a separate `mcp.json` and creates stdio registrations from `type`-discriminated server config; an omitted `type` defaults to stdio. That factory constructs one `StdioTransport` and stable `McpClient` per server, inherits only a small baseline environment before merging the server's explicit `env`, and forwards stderr, client errors, and manager errors to the current session logger. Before constructing the app, the TUI starts the manager, reserves the complete built-in tool namespace, and injects discovered tools into every recreated main Agent; memory-consolidation Agents never receive these external tools. During shutdown, the app first cancels and awaits the active Agent, then product composition closes the manager.
