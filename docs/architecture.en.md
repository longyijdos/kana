...
- `kana exec [--clean] [prompt...]` / `kana exec resume <sessionId> [prompt...]`: runs one complete Agent turn without the TUI and exits; `--json` writes a versioned JSONL event stream.
...
`OpenAICodexModel` uses the ChatGPT token and account ID supplied by Kana's generic OAuth state machine to send `store = false` SSE requests to the Codex Responses Lite endpoint. The adapter maps reasoning-summary, provider-hosted `web_search_call`, message, and function-call output items into the same ordered content protocol, persisting encrypted reasoning and completed items as opaque `providerState` for replay on later turns. Hosted searches never enter the local ToolRuntime. The first `401` refreshes credentials and retries once. Subscription usage records tokens without applying Platform API pricing. See [OpenAI Codex provider adapter](openai-codex-provider.en.md).
...
Clean mode retains only items 2 and 5. It does not scan Skill paths, read memory, or construct the automatic memory-consolidation scheduler. The host still supplies current runtime configuration to the Agent, so provider/model, context limit, output ceiling, and tool deadline remain identical to normal mode; `schedule_wake` also remains available when supported by the frontend. Agent configuration changes in clean mode pass through the same schema validation but replace only the host's in-process configuration without invoking the shared `KanaConfigStore`.
...
`ConversationRuntime` is the product-level conversation lifecycle boundary. It owns the current Agent and session, rejects concurrent submissions, centralizes new/fork/resume and Agent replacement after configuration or tool changes, and drains the current session's pending-submission FIFO when the frontend permits. Tab input, due wakes, and deferred steering share this new-run queue, while Enter steering first enters the Agent's run-local queue. The runtime retains a stable ID, origin, and safe display text for every pending item; scheduled items also retain their due time and are published with the in-process scheduler's future-wake list as a read-only snapshot. It exposes the current session's user-scheduling and ID-based cancellation boundary. Cancellation synchronously checks both future timers and due pending items, so execution order, management state, and TUI display cannot diverge. It publishes frontend-neutral run, Agent-event, input-queue, and session-change events; listener failures are isolated and recorded as diagnostics. `KanaTuiApp` owns only accumulated usage/cost and interaction controllers, subscribes to runtime events, and delegates their visible mapping to `AgentEventRenderer`. `QueuedInputController` stores only visible run-local `next turn` items and projects the runtime snapshot into `next run`, due `scheduled`, and future-wake summary rows; `ScheduledMessageManagerController` owns `/schedule`'s static management snapshot and multi-step add/delete flow; `ExternalToolsLifecycleController` owns initial loading, MCP reload, progress-block, and input-restoration state; `SlashCommandController` centralizes command routing and argument validation; and `SessionLifecycleController` coordinates transcript, focus, and status resets after new/fork/resume. They request cross-domain actions through narrow callbacks instead of mutating app state in reverse.
