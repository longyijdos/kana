...
When the TUI starts, `startTui` loads runtime configuration, the separate MCP configuration, and the approval allowlist, then constructs `KanaTuiApp` with an idle MCP manager. Only after the current session is known and the first TUI view is displayed does the app invoke its injected external-tool loader to connect MCP servers, discover the initial tools, and rebuild the main Agent. The `kana resume` picker therefore does not start MCP; loading begins after a session is selected. Session I/O, Skill activation, memory compaction, external-tool loading, and the Agent factory are all injected as callbacks. The app therefore coordinates user flows without knowing JSONL, TOML, MCP transports, or other storage and protocol details.
