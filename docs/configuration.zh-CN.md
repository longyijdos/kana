...
| `image_input` | 布尔值 | `true` | 将会话中持久化的用户图片作为 classic Responses `input_image` data URL 发送。设为 `false` 时仍保留会话附件，但模型输入会改为明确的省略提示。 |
...
| `tui.group_tool_calls` | 布尔值 | `true` | 是否把连续纯工具模型回合中的 list/glob/grep/read 合并为一个探索动作，并把相邻 provider-hosted 网页动作合并为一个搜索动作；关闭时逐条显示。 |
...
`hyperlinks` 是功能许可而不是强制开关：即使配置为 `true`，Kana 也只对确认支持 OSC 8 的终端启用，无法确认能力时保持可见 URL；配置为 `false` 时始终使用文本 fallback。`smooth_text_streaming` 默认只调整可见文本的推进节奏，不会向 provider 或 Agent 施加背压；关闭后仍由 TUI 合并终端重绘，但不再拆分 provider 的文本快照。`collapse_long_pastes` 只影响编辑器的显示与编辑方式，提交、排队和从输入历史恢复时仍使用完整粘贴原文。`group_tool_calls` 只影响分组；无论是否启用，provider reasoning 与本地工具参数流都共用一个 `thinking` 动作，本地工具块只在真正开始执行时出现。启用分组后，连续纯工具模型回合会把正在执行的 list/glob/grep/read 追加到同一个探索阶段；只要 Agent 仍可能继续探索，当前调用全部完成也不会让该阶段进入终态。探索计时不包含参数解析、排队和审批等待；后面存在已排队的非探索工具时，最后一个探索调用完成后会在该工具等待审批前冻结阶段。可见助手正文、非探索工具、失败的探索调用、上下文压缩或 Agent run 结束也会让该阶段进入终态；失败项仍保留独立错误输出。合并后的 read 默认显示最终文件名，发生冲突时使用最短唯一后缀。hosted 网页分组先显示空的活动标题，只追加 provider 已完成的 action，因此不会闪现 provisional 的通用 `Search`；同一响应中的 reasoning 和后续 hosted action 会保持活动态，直到可见助手正文、本地工具或响应结束建立终态边界。未完成 action 在用户中止时显示 `Web search stopped`，在 provider 失败时显示 `Web search failed`，从历史恢复时也遵循相同规则。`daily_retention_days` 注释掉或省略时不会清理每日记忆。日志固定写入 `<KANA_HOME>/logs`，不提供目录配置，也不写入终端输出，因而不会干扰 TUI 重绘。`max_turns` 只接受 `-1` 或正整数；`parallel_tool_calls`、供应商 `web_search`/`image_input`、`hyperlinks`、`smooth_text_streaming`、`collapse_long_pastes` 和 `group_tool_calls` 必须是布尔值；`tool_deadline_ms`、`max_tokens` 和可选的 `context_limit` 要求正整数，`timeout_ms` 和 `max_retries` 校验为有限数字，`memory` 的两个数量字段要求正整数。
