...
| `tui.group_tool_calls` | 布尔值 | `true` | 是否把相邻 list/glob/grep/read 调用合并为一个 `Explored` 动作，并把相邻 provider-hosted 网页动作合并为一个搜索动作；关闭时逐条显示。 |
