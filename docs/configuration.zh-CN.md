...
可重复指定的 `--set <path=value>` 为 TUI、`resume`、`exec` 和 `exec resume`（包括 clean 启动）覆盖当前进程的启动配置。路径使用点分隔的 TOML 裸键，值必须是合法 TOML；字符串需要通过 Shell 引号保留 TOML 引号。覆盖按传入顺序应用，同一路径最后一次赋值生效；数组和内联表替换对应路径的值。未知字段与 `config.toml` 一样被忽略，赋值格式错误或已知字段的非法值则导致启动失败。文件会先于覆盖进行校验，因此不能用覆盖修复非法文件配置。
