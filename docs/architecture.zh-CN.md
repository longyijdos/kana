...
`loadKanaConfig` 通过 provider 传输设置、具体模型默认值和独立的主/Memory Agent override 解析可选 `config.toml`。动态 TOML 形状在此边界结束；Agent 与 Model 构造函数只接收解析后的判别联合类型。适用的非法值和已知表中的未知键会报错；对所选模型 inactive 的已知模型专用 override 会保留且不校验。Install 不物化默认 `config.toml`，生成的 example 仍是运行时不读取的参考。`KanaConfigStore` 只提供主 Agent 模型选择更新：它精确 patch `[agent]` leaf，在 commit 前解析并构造候选对象，再通过同目录临时文件原子替换，同时保留无关表、inactive 值和注释。
...
记忆分 global 和 project 两个 scope。`remember` 先向当天的暂存文件追加结构化条目；对话提交后，调度器按 scope 启动增量压缩 Agent。增量压缩和手动全量压缩共享每个 scope 的队列，串行执行该 scope 的全部读—改—写任务。压缩 Agent 使用独立解析的 `[memory.agent]` 模型与运行限制，并且只有记忆读写工具；它在助手以正常 `stop` 结束时才提交内存中的修改。主 Agent 的 `/model` 变更不会重建该 scheduler。通过 `/memory` 交互流程选择 Compact 可发起全量压缩，并在成功后按 `daily_retention_days` 清理过期每日记忆。
