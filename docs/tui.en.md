...
The editor owns the status line, which shows the model and reasoning effort (for example, `gpt-5.6-luna · max`; DeepSeek shows `off` when thinking is disabled), the clean-mode marker, the latest assistant prompt's percentage of the effective context limit, run phase, active tool, and cwd. Multiple parallel tools collapse to the first name plus a remaining count, such as `tool read +2`. After any call fails, the error phase remains until the group finishes, while a completed call never clears names that are still active. The phase is `compacting` while a context summary is generated; completion immediately updates the percentage from the checkpoint estimate, and later normal model usage replaces that estimate. While input is queued, the editor uses rows below the status line that Layout would otherwise pad to display `Queued inputs`, marking delivery as `next turn` or `next run`. Multiline content collapses to one row, and constrained space shows the first items plus a remaining count. Opening the slash-command palette hides both status and queue preview. Replacing the editor with another bottom component hides input, status, and preview together. Every completed assistant and summary request contributes to process totals and CNY cost, but summary usage does not become the normal prompt's context percentage; `/usage` counts turn-limit termination separately from normal completion, output truncation, abort, and failure.
...
- `QueuedInputController` owns display IDs, delivery lanes, and ordering for TUI queue items. When the Agent reports steering as deferred, it moves that item from `next turn` to the shared FIFO's `next run` tail.
