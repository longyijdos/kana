...
Assistant messages and the memory viewer use lightweight Markdown rendering: headings, lists, quotes, fenced code, some inline styles, table rows, link/image text, and limited HTML normalization. Shiki highlighting preloads in the background; code is plain text until it is ready. Tool blocks display list/glob/grep/read summaries, highlighted write/edit diffs, and tail bash output; write approvals and tool blocks distinguish new-file writes from overwrites; long output opens in the scrollable viewer.
