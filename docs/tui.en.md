...
- `McpServerManagerController` replaces the editor with configured MCP server checkboxes. `Enter` edits only a local draft; `Esc` applies it. A changed draft is persisted once and triggers one full runtime reload, while an unchanged draft just closes. Persistence errors keep the view open. The component displays the server ID, transport, and full command line (`command` plus `args`), but never receives environment values.
