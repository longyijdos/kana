...
In clean mode `/skills`, `/mcp`, and `/memory` remain discoverable commands but report an explicit unavailable error when invoked; they cannot re-enable custom content in the current process. `/model`, session commands, `/compact`, `/usage`, and the local Shell continue to work normally.
