...
With `kana --clean`, the app installs no external-tool loader and constructs no MCP management controller, so initial display, session resume, and later Agent rebuilds never read or connect MCP. The transcript shows one clean-mode notice and the status line keeps `clean` visible.
...
In clean mode `/skills`, `/mcp`, and `/memory` remain discoverable commands but report an explicit unavailable error when invoked; they cannot re-enable custom content in the current process. `/approval`, `/model`, session commands, `/compact`, `/usage`, and the local Shell continue to work normally.
...
- `/model` completes a switch only while idle. Kana retains the current messages and context checkpoint, builds a candidate Agent and memory-consolidation scheduler from the new configuration, then atomically persists only the configuration fields that changed. It replaces the current Agent and updates the model and reasoning effort in the status line only after every step succeeds. Construction or persistence failures preserve the previous Agent and configuration and add an error to the transcript. The selection becomes active for later new, forked, or resumed sessions and compaction work; later turns in one session may use different providers, while per-run accounting still records the model actually used.
