...
Assistant messages and the memory viewer use lightweight Markdown rendering: headings, lists, quotes, fenced code, some inline styles, tables, link/image text, and limited HTML normalization. Tables are parsed as blocks with optional boundary pipes, empty cells, escaped pipes, and column alignment; widths use visible terminal columns and narrow layouts fall back to vertical key/value records. While streaming, only completed rows determine column widths, the growing tail row previews within the current layout, and message completion finalizes it. Paired HTML tags and void tags are removed, while unmatched programming syntax such as `vector<int>` is preserved verbatim. Shiki highlighting preloads in the background; code is plain text until it is ready. Tool blocks display list/glob/grep/read summaries, highlighted write/edit diffs, and tail bash output; write approvals and tool blocks distinguish new-file writes from overwrites; long output opens in the scrollable viewer.
