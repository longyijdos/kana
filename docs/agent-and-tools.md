...
Every new tool result's model-visible `content` is uniformly capped at `min(16000, max(256, floor(promptBudget × 25%)))` estimated tokens, using three UTF-8 bytes per estimated token as the exact final byte guard. In Kana's default artifact policy, oversized non-`read` content is saved completely before a roughly 70% head / 30% tail preview is built. Its retrieval notice, exact omitted-byte count, and locator remain inside that same byte guard. Top-level `read` results use a bounded offset/limit hint without recursively spilling. The canonical structured result remains available on the live `tool_execution_end` event, while oversized, non-serializable, or artifact-backed structured data is omitted from the durable message.
