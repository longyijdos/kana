...
| `bash` | `command`, optional `cwd`, `timeoutMs` (1–600000), and `background` (default `false`) | Foreground execution defaults to a 30000 ms timeout and returns complete final stdout/stderr. `background: true` has no default timeout and immediately returns a session-owned Job ID. |
...
| `job_output` | `jobId`, optional `waitMs` (0–30000; default 0) | Consumes all currently unseen retained output from the Agent cursor in one call. It may wait for output or a terminal state without stopping the Job and reports dropped bytes explicitly. |
