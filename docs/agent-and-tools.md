...
TUI 启动时从独立 `mcp.json` 读取 `mcpServers`、启动 stdio manager，并把发现的远端工具作为 `additionalTools` 注入主 Agent。`/new`、`/fork`、`/resume` 和 Skills 刷新重建 Agent 时会复用同一份启动期工具集合；记忆压缩 Agent 不经过这条工厂，因此不会获得 MCP 工具。`McpManager` 只要求 client 实现 `connect/listTools/callTool/close`，工具适配器只要求 `McpToolCaller`；稳定版 stdio client 和后续无状态、Streamable HTTP 或 SSE client 可以继续共用管理与工具边界。
