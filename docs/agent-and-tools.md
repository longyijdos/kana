...
| `bash` | `command`, optional `cwd`, optional `timeoutMs` (1–600000; default 30000) | Runs through the user's shell in login-command mode and returns exit code, complete final stdout/stderr, and timeout state. Live updates expose bounded trailing snapshots of each stream. |
