...
| `tool` | `id`, `provenance: { kind: "tool_result" }`, `toolCallId`, `toolName`, `content`, optional `images`, `result`, `isError` | Associates one text or visual tool result back to the model. |
...
Tool results have two layers: `content` is text for the model, while `result` preserves the original structured value for the Agent, TUI, and persistence. An optional provider-neutral `images: UserImage[]` carries native visual observations alongside the text and is committed to the corresponding tool-result message. If a tool returns an ordinary value directly, the runtime uses a string unchanged or JSON-serializes another value for `content`, while retaining that value as `result`.
