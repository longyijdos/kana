...
The generic Agent layer accepts one optional `ToolResultPolicy`. ToolRuntime invokes its `finalize()` exactly once after every outcome has become a normalized `ToolResult`, including success, missing tools, argument failures, approval denial, cancellation, timeout, and exceptions. The policy receives deep-cloned, read-only views of the model-authored call and normalized result. It may preserve the model-visible result text, replace only that text, and append source-attributed internal user-role context after the result; it cannot rewrite the tool name, arguments, structured host result, or error state. Policy exceptions and invalid returns emit safe diagnostics and fall back to the original result.
