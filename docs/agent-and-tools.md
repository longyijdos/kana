...
| `tool` | `id`, `provenance: { kind: "tool_result" }`, `toolCallId`, `toolName`, `content`, optional `images`, `artifact`, `result`, `isError` | Associates one text or visual tool result back to the model. |
...
Tool results have two primary layers: `content` is text for the model, while `result` is the original structured value used by live Agent/TUI consumers and retained durably only when it is safe to do so. An optional `artifact: { kind: "text", locator, byteLength }` is bounded presentation metadata for complete text stored outside the message, and provider-neutral `images: UserImage[]` carries native visual observations alongside the text. If a tool returns an ordinary value directly, the runtime uses a string unchanged or JSON-serializes another value for `content`, while keeping that value as the canonical live `result`.
...
Repeat (at most 8 turns by default; unlimited when maxTurns = -1):
  ...
  Commit and append changed explicit runtime-context state transitions
...
When a `ContextManager` is configured, every model request first receives a separate model projection of the full Agent history; compaction never deletes the original `messages`. Between checkpoints, that projection retains every runtime-context transition. The stable system protocol makes only the last message for each source authoritative, so changed and inactive states append without rewriting the preceding model-message prefix. Runtime-context messages are excluded from summary-policy input because they are authoritative state rather than conversation. At a checkpoint boundary, only the last state for each source that is active at that boundary is reprojected immediately after the summary; every later transition stays in original order. The prompt budget is the context limit minus the safety reserve, with no fixed reservation for configured maximum output. Reaching 80% of that budget triggers compaction. Rules scan oldest to newest and may cut only after a complete assistant turn without tool calls, or after every result for one assistant tool-call group has appeared. The first boundary whose “maximum summary placeholder + boundary runtime state + recent raw messages” fits the 10% target is selected, allowing one compaction to cover as much old context as possible. With no safe boundary, compaction is deferred while still within the prompt budget and fails when recovery cannot proceed safely.

Prompt estimates distinguish replayable context from per-response billing usage. After a response without provider-hosted tools, the manager keeps the provider's `input_tokens` as an exact anchor for that request and adds local estimates only for messages committed afterward. A response containing a hosted tool never replaces that anchor: hosted search pages and similar transient provider material can be included in billed input without being present in Kana's replayable history. Until another clean response calibrates the estimate, Kana counts only persisted assistant content, hosted-call metadata, client tool calls/results, and later user messages. A new, switched-model, or freshly compacted context without an anchor is estimated locally from its model projection. Resuming a session rebuilds the anchor in memory from the latest persisted assistant response: the response's position in history becomes the message count and its provider `input_tokens` become the anchor, so only messages committed after that response are estimated locally. Responses without usage or with hosted tools are skipped, and responses recorded before the current compaction checkpoint are rejected because they were measured against a different projection; without a valid anchor the full local estimate remains the fallback. Text uses a conservative UTF-8 byte estimate, protocol envelopes add fixed overhead, tool schemas/actions are estimated from JSON, and images use patch counts instead of stored base64 size.
...
An injected `CompactPolicy` produces the actual summary. Kana's product policy calls the main Agent's same `Model` once with tool-free `generate()`; it does not start another Agent loop. Input contains the previous summary and newly covered messages. Assistant thinking, assistant usage, and a tool's structured `result` are omitted, while model-visible tool `content`, name, error state, and visual observations remain. The summary must finish with `stop` and fit its summary budget; failure restores the preceding checkpoint.

Every new tool result's model-visible `content` is uniformly capped at `min(16000, max(256, floor(promptBudget × 25%)))` estimated tokens, using three UTF-8 bytes per estimated token as the exact final byte guard. In Kana's default artifact policy, oversized non-`read` content is saved completely before a roughly 70% head / 30% tail preview is built. Its retrieval notice, exact omitted-byte count, and locator remain inside that same byte guard. Top-level `read` results use a bounded notice without recursively spilling; it states that offset/limit pagination is line-based and cannot page within one very long line. The canonical structured result remains available on the live `tool_execution_end` event, while oversized, non-serializable, or artifact-backed structured data is omitted from the durable message even if artifact storage fails.
...
`Agent.stream(input)` starts the loop asynchronously. `AgentConfig.promptAssembly` is mutually exclusive with the legacy `system`/`tools` inputs; those legacy inputs are converted to a single immutable assembly for compatibility. Every configured context renderer must return an explicit, non-empty active or inactive state. An initially inactive source writes nothing; after activation, changed states follow the same write-before-model rule as run input. Generic embedders without a journal retain in-memory behavior. The Agent permits only one active run; concurrent attempts receive an error stream. `prompt(input)` is the convenience form that awaits `stream(input).result()`.
...
| `view_image` | `path` | Loads and normalizes a local image, returns deterministic path/format/dimension/byte metadata, and attaches the image as a visual observation for the same active model. Registered only when that model supports images and `image_input` is enabled. |
...
| `bash` | `command`, optional `cwd`, `timeoutMs` (1–600000), and `background` (default `false`) | Used when no purpose-built tool directly covers the operation. Foreground execution defaults to a 30000 ms timeout and returns complete final stdout/stderr. `background: true` has no default timeout and immediately returns a session-owned Job ID; use it instead of raw shell backgrounding when work must outlive the call. |
| `job_list` | None | Lists active and up to 32 recent terminal Jobs owned by the current session. Reading a terminal entry acknowledges its completion notification. |
| `job_output` | `jobId`, optional `waitMs` (0–30000; default 0) | Consumes all currently unseen retained output from the Agent cursor in one call. It may wait for expected output or a terminal state without stopping the Job and reports dropped bytes explicitly; otherwise the Agent should not repeatedly poll a running Job. |
| `job_kill` | `jobId`, optional `reason` | Stops an owned Job, waits for its process group to become quiescent, and returns its terminal state. |
| `todo_write` | Complete `items` array of non-empty `content` plus `pending`, `in_progress`, or `completed` status | Atomically replaces the current session todo list for multi-step work. At most one item may be `in_progress`; an empty array explicitly clears the list. The model-facing result is a fixed compact acknowledgement. |
...
`bash` always disconnects stdin and defines `sudo` as `sudo -n`, preventing password prompts from taking over TUI input. Foreground execution emits partial stdout/stderr roughly every 100ms; each live snapshot is a bounded trailing window of at most 20,000 JavaScript characters per stream, so long-running commands show the freshest output instead of the beginning of the stream. The complete final streams then enter the shared result policy, which may store them as an artifact before bounding model-facing content and durable structured data. Each command runs in a separate process group. Foreground execution waits for that group, not just the top-level shell, so raw `command &` no longer escapes: the default or explicit timeout terminates the whole group. Explicit daemonization that creates another process session can still leave Kana's process-group boundary. A non-zero exit code describes the command outcome and does not set the tool result's `isError` flag. A timeout records a `null` exit code and marks the result as an error.

The generic `BackgroundJobManager` and Job tools do not depend on Kana's Agent construction and can be composed by another host. An owner binds Jobs to one session instance, enforces its concurrency limit, and removes all of its Jobs during disposal. Job metadata stores a whitespace-normalized label of at most 512 UTF-8 bytes; the original Bash command remains in the tool call. Each Job keeps only its latest 1 MiB of stdout/stderr in memory. `job_output` consumes all currently unread retained output from one implicit Agent cursor in a single call, so a completed Job needs at most one read; `droppedBytes` reports unread data evicted before consumption. Oversized model-facing output is bounded by the shared result policy. TUI preview uses a separate non-consuming trailing window of at most 20 KiB. The manager retains at most 32 terminal Jobs per owner and prunes the oldest. Job manager state and retained output buffers are not persisted or resumed after process exit.

Kana adds runtime context, completion delivery, session cleanup, and TUI management around that generic layer. Runtime context contains only active or unreported Job identity, bounded command label, cwd, status, and exit code—never output—and the Agent adds a new runtime-context message only when that state changes. A completion becomes `next-step` when an Agent run is active or `next-turn` when idle; adjacent pending completions are coalesced into one run without crossing other queued input. Ordinary output does not wake the Agent, so a continuing development server stays running without repeated wakes. Observing a terminal Job through `job_list`, `job_output`, or `/jobs` acknowledges it and cancels a still-queued completion input. Session switch and orderly shutdown stop owned process groups before later host resources such as MCP close; a forced second interrupt may bypass that cleanup.

`list`, `glob`, `grep`, `read`, `view_image`, `write`, `edit`, and `bash` resolve relative paths against the tool `root` (Kana's startup directory) and accept absolute paths. They are not workspace sandboxes: relative paths may escape the root, symlinks may resolve outside it, and `bash.cwd`, `glob.cwd`, and `grep.path` may also be outside. `view_image` uses the same decoder and normalization limits as user attachments; animated images such as GIF are represented by their decoded first frame and normalized to a static PNG. Treat approval as interactive confirmation, not filesystem isolation.

`todo_write` trims item content, rejects blank or duplicate content after trimming, rejects unknown fields, and never partially changes state on validation failure. Accepted state is session-owned and durable; a new human turn and an all-`completed` list do not clear it. The latest complete list is projected into model runtime context even after compaction, resume, or fork, while the tool acknowledgement stays independent of list size. Only an explicit empty replacement clears the current state. Historical accepted snapshots remain attached to their original tool calls. The tool is exclusive and does not require approval.
